...
fun palavraRecebida(palavra:String):Boolean{
    var palavra2 = palavra.uppercase()

    var condition:Boolean = false//Condição que retorna

    var pl = palavra2.length //Tamanho total da palavra
    ...
    while(n < pl){ // Se a palavra tiver 20 caracteres vai rodas 20 vezes
        ...
        if (palavra2[n] == 'X'){ //Caso o caractere em questão tenha x, ADICIONA 1 ao número X
            numX += 1
        }
        if (palavra2[n] == 'O'){ // Caso o caractere em questão tenha o, adiciona 1 ao número O
            numO += 1
        }
